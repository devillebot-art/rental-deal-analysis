...
def get(path):
    ...
    # download_url gives direct raw content
    download_url = data.get("download_url", "")
    if download_url:
        try:
            raw_resp = urllib.request.urlopen(download_url, timeout=15)
            content = raw_resp.read()
            if content:
                return content.decode("utf-8")
        except Exception as e:
            print(f"download_url failed: {e}")
    # Fallback: decode base64
    content_b64 = data.get("content", "")
    if content_b64:
        return base64.b64decode(content_b64.replace("\n","")).decode("utf-8")
    print(f"WARNING: could not get content for {path}")
    return None

def put(path, content_str, msg):
    url = f"https://api.github.com/repos/{REPO}/contents/{path}"
    sha_resp = json.loads(urllib.request.urlopen(
        urllib.request.Request(url + "?ref=main", headers=H), timeout=15).read())
    sha = sha_resp["sha"]
    payload = json.dumps({
        "message": msg,
    ...
    return json.loads(urllib.request.urlopen(req, timeout=15).read()).get("commit",{}).get("sha","")[:8]

raw = get("data/properties.json")
if not raw:
    print("ERROR: could not load properties.json")
    exit(1)
...
def card(p, show_date=False):
    cf  = p.get("cash_flow",{}); sc = p.get("scores",{}); v = p.get("verdict","SKIP")
    score=sc.get("overall_score",0); cfv=cf.get("monthly_cash_flow",0)
    pos=cf.get("breakdown",{}).get("positives",[]); neg=cf.get("breakdown",{}).get("negatives",[])
    dp=cf.get("down_payment",150000); req_dp=cf.get("required_down_payment_for_20pct_cf")
    ...
    addr=e(p.get("address","Unknown")); city=e(p.get("city","") or ""); pt=e(p.get("property_type",""))
    cond=e(p.get("condition","")); src=p.get("source","View").upper()
    ts=p.get("analyzed_at","")[:16].replace("T"," ") if show_date else ""
    mr=cf.get("monthly_rent_estimate",0); mm=cf.get("mortgage_payment",0)
    mt=cf.get("monthly_tax",0); mi=cf.get("monthly_insurance",0)
    mmt=cf.get("monthly_maintenance",0); mv=cf.get("monthly_vacancy",0)
    tot_exp=cf.get("total_monthly_expenses",0)
    pos_h="".join(f"<li>{e(x)}</li>" for x in pos[:6])
    neg_h="".join(f"<li>{e(x)}</li>" for x in neg[:6])
    cc="cf-pos" if cfv>=0 else "cf-neg"
    cb=f'<span class="badge bg-muted">{cond}</span>' if cond else ""
    ...
    if req_dp is not None:
        ...
        req_dp_html='<span class="reqdp-na">&#x26F0; Not achievable — price too high for corridor</span>'

    return f'''<div class="card">
  <div class="card-header">
    <div><div class="card-title">{addr}</div><div class="card-sub">{city}{" · "+pt if pt or city else ""}</div></div>
    <span class="badge bg-{bc}">{v}</span>
  </div>
  <div class="price-row">
    <span class="price-big">${p.get("price",0):,}</span>
    <span class="beds-baths">{beds}BR / {baths}BA{" · "+str(sqft)+" sqft" if sqft else ""}</span>
  </div>
  <div class="card-badges">
    <span class="badge bg-yellow">Score: {score}</span>
    <span class="badge bg-{"green" if cfv>=0 else "red"}">${cfv:,}/mo CF</span>
  ...
  <div class="cf-block">
    <div class="cf-col">
      ...
      <table class="cf-table">
        <tr><td>Rent</td><td class="cf-pos">+${mr:,}</td></tr>
        <tr><td>Mortgage (P&amp;I @ 7%)</td><td class="cf-neg">&#8722;${mm:,.0f}</td></tr>
        <tr><td>Property Tax</td><td class="cf-neg">&#8722;${mt:,.0f}</td></tr>
        <tr><td>Insurance</td><td class="cf-neg">&#8722;${mi:,.0f}</td></tr>
        <tr><td>Maintenance (10%)</td><td class="cf-neg">&#8722;${mmt:,.0f}</td></tr>
        <tr><td>Vacancy (8%)</td><td class="cf-neg">&#8722;${mv:,.0f}</td></tr>
        <tr class="cf-total"><td>Cash Flow</td><td class="{cc}">${cfv:,.0f}/mo</td></tr>
      </table>
    </div>
    <div class="cf-col">
      ...
      <table class="cf-table">
        <tr><td>Target CF (20% of rent)</td><td class="cf-pos">+${int(round(mr*0.20,-1)):,}</td></tr>
        <tr><td>Monthly Non-Mortgage</td><td class="cf-neg">&#8722;${tot_exp - mm:,.0f}</td></tr>
        <tr class="cf-total"><td>Max Mortgage Allowable</td><td class="cf-neg">${int(round(mr*1.20-(tot_exp-mm),-1)):,}/mo</td></tr>
      </table>
    </div>
  </div>
  <div class="breakdown-grid">
    <div><div class="breakdown-header cf-pos">&#x2705; Positives</div><ul class="breakdown-list">{"<li>None identified</li>" if not pos_h else pos_h}</ul></div>
    <div><div class="breakdown-header cf-neg">&#x26A0; Negatives</div><ul class="breakdown-list">{"<li>None identified</li>" if not neg_h else neg_h}</ul></div>
  </div>
  <div class="verdict-banner {vc}">{v} — {e(p.get("verdict_reason",""))}</div>
  <div class="card-footer">
    <a href="{e(p.get("url","#"))}" target="_blank">{src} &#x2197;</a>
...
.reqdp-box{font-size:0.76rem;font-weight:600;padding:0.5rem;border-radius:6px;margin-bottom:0.4rem;line-height:1.5}
...
top_html="\n".join(card(p) for p in top) if top else '<div class="empty">No properties yet!</div>'
rec_html="\n".join(card(p, True) for p in recent) if recent else '<div class="empty">No analyses yet.</div>'
...
<body>
<div class="container">
<header><h1>🏠 Rental Deal Analyzer</h1><span class="updated">Updated: {now} &nbsp;|&nbsp; Auto-updated from email</span></header>
<section><h2>⭐ Top Opportunities — Ranked by Cash Flow at $150K Down</h2>
<div class="cards">{top_html}</div></section>
<section><h2>📋 Recent Analyses</h2>
<div class="cards">{rec_html}</div></section>
<footer><p>⚡ Johnny5 &nbsp;|&nbsp; Default: $150,000 down payment &nbsp;|&nbsp; Target: 20% cash flow over expenses &nbsp;|&nbsp; Not financial advice.</p></footer>
</div></body></html>"""

sha=put("index.html", html, f"Rebuild — {now}")
print(f"HTML pushed: {sha} | {len(html)} bytes")
