...
def save_file(path, content, msg):
    ...
    result = github_api("PUT", f"repos/{GITHUB_REPO}/contents/{path}", payload)
    return result
...
def save_properties(data):
    ...
    return save_file(DATA_PATH, content,
        f"Update properties — {datetime.now().strftime('%Y-%m-%d %H:%M')} UTC")
...
def extract_links(text):
    ...
    property_domains = [
        ...
        "movoto", "property", "listing", "REA", "rltre", "realestate.com"
    ]
    found = []
...
def build_html(properties):
    ...
    # Top opportunities: top 5 by overall score
    top = sorted(
        [p for p in properties if p.get("status") == "active"],
        key=lambda x: x.get("scores", {}).get("overall_score", 0),
        reverse=True
    )[:5]

    # Recent: last 10 analyzed
    recent = sorted(properties, key=lambda x: x.get("analyzed_at", ""), reverse=True)[:10]

    def make_property_card(p, show_date=False):
        cf = p.get("cash_flow", {})
        scores = p.get("scores", {})
        verdict = p.get("verdict", "SKIP")
        ...
        vclass = {"BUY": "v-buy", "CONDITIONAL": "v-cond", "SKIP": "v-skip"}.get(verdict, "v-skip")
        bc = {"BUY": "green", "CONDITIONAL": "yellow", "SKIP": "red"}.get(verdict, "red")
        bar_class = "high" if score >= 75 else "mid" if score >= 55 else "low"

        sqft = p.get("sqft")
        beds = p.get("beds", "?")
        baths = p.get("baths", "?")
        address = escape(p.get("address", "Unknown"))
        city = escape(p.get("city", "") or "")
        prop_type = escape(p.get("property_type", ""))
        condition = escape(p.get("condition", ""))
        feats = p.get("features", [])

        # Build positive/negative lists (max 6 each)
        ...
        cf_class = "cf-pos" if cf_val >= 0 else "cf-neg"

        link_text = p.get("source", "View").upper()
        analyzed = p.get("analyzed_at", "")[:16].replace("T", " ") if show_date else ""

        return f'''<div class="card">
  <div class="card-header">
    <div>
      <div class="card-title">{address}</div>
      <div class="card-sub">{city}{" · " + prop_type if prop_type or city else ""}</div>
    </div>
    <span class="badge bg-{bc}">{verdict}</span>
  </div>

  <div class="price-row">
    <span class="price-big">{fmt_currency(p.get("price"))}</span>
    <span class="beds-baths">{beds}BR / {baths}BA{f" · {sqft:,} sqft" if sqft else ""}</span>
  </div>

  <div class="card-badges">
    <span class="badge bg-yellow">Score: {score}</span>
    <span class="badge bg-{'green' if cf_val >= 0 else 'red'}">{fmt_currency(cf_val)}/mo CF</span>
    <span class="badge bg-yellow">CoC: {cf.get('cash_on_cash_return_pct', 0)}%</span>
    {f'<span class="badge bg-muted">{condition}</span>' if condition else ''}
  </div>

  <div class="score-bar"><div class="score-fill {bar_class}" style="width:{score}%"></div></div>

  <table class="cf-table">
    <tr><th colspan="2">Monthly Cash Flow Estimate</th></tr>
    ...
    <tr class="cf-total"><td>Cash Flow</td><td class="{cf_class}">{fmt_currency(cf_val)}/mo</td></tr>
  </table>
  ...
  <div class="verdict-banner {vclass}">{verdict} — {escape(p.get("verdict_reason", ""))}</div>

  <div class="card-footer">
    <a href="{escape(p.get("url", "#"))}" target="_blank">{link_text} ↗</a>
    {f'<span class="date">{analyzed} UTC</span>' if show_date else ''}
  </div>
</div>'''

    top_html = "\n".join(make_property_card(p) for p in top) if top else '<div class="empty">No properties yet — email a listing link to get started!</div>'
    rec_html = "\n".join(make_property_card(p, show_date=True) for p in recent) if recent else '<div class="empty">No analyses yet.</div>'
...
<style>
:root {{ --bg: #0d1117; --surface: #161b22; --border: #30363d; --text: #e6edf3; --muted: #8b949e; --accent: #58a6ff; --green: #3fb950; --red: #f85149; --yellow: #d29922; --m5: #6366f1; }}
...
.cards {{ display: grid; grid-template-columns: repeat(auto-fill, minmax(420px, 1fr)); gap: 1.2rem; }}
...
.card-header {{ display: flex; justify-content: space-space-between; align-items: flex-start; margin-bottom: 0.8rem; gap: 0.5rem; }}
...
.cf-table {{ width: 100%; border-collapse: collapse; margin: 0.8rem 0; font-size: 0.78rem; }}
.cf-table th {{ background: var(--bg); color: var(--accent); padding: 0.4rem 0.6rem; text-align: left; border: 1px solid var(--border); font-size: 0.75rem; }}
.cf-table td {{ padding: 0.3rem 0.6rem; border: 1px solid var(--border); }}
.cf-table td:last-child {{ text-align: right; font-weight: 600; }}
.cf-total td {{ border-top: 2px solid var(--border); font-weight: 700; font-size: 0.85rem; }}
...
@media (max-width: 600px) {{ body {{ padding: 1rem; }} .cards {{ grid-template-columns: 1fr; }} .breakdown-grid {{ grid-template-columns: 1fr; }} }}
</style>
</head>
<body>
<div class="container">
...
<footer>
  <p>⚡ Powered by Johnny5 &nbsp;|&nbsp; Estimates based on FL market data — verify independently</p>
  <p>Not financial advice. For rental analysis only.</p>
</footer>
</div>
</body>
</html>'''
    ...
    for msg_ref in messages:
        ...
        for url in links:
            ...
            if result["status"] == "ok":
                ...
                v = prop["verdict"]
                s = prop["scores"]["overall_score"]
                print(f"  ✅ {v} ({s}) — {prop['address']}")
            else:
